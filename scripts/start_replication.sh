...
if [[ -z "$DATABASES" ]]; then
  DATABASES=$(mysql -h ${rds_snapshot_instance} \
    -u ${rds_backup_username} \
    --password=${rds_backup_password} \
    -NBe 'show schemas' | grep -wv 'mysql\|information_schema\|performance_schema')
fi

# Dump database
echo "Copying databases to intermediate instance"
mysqldump \
  -h ${rds_snapshot_instance} \
  -u ${rds_backup_username} \
  --password=${rds_backup_password} \
  --databases $DATABASES \
  --order-by-primary \
  ...
  --single-transaction | mysql -u root --password=${intermediate_root_password}

# Set master status
echo "Setting master status"
mysql \
  -u root \
  --password=${intermediate_root_password} \
  -e "CHANGE MASTER TO MASTER_HOST = '${rds_original_instance}', 
  MASTER_PORT = 3306, 
  MASTER_USER = '${rds_replication_username}', 
  MASTER_PASSWORD = '${rds_replication_password}', 
...
mysqldump \
  ...
  --single-transaction \
  --set-gtid-purged=on \
  --master-data=1 | gzip | gsutil cp - gs://${gcs_bucket}/dump.sql.gz
